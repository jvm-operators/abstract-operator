...
public abstract class AbstractCustomResourceWatcher<T extends EntityInfo> extends AbstractWatcher<T> {

    protected static final Logger log = LoggerFactory.getLogger(AbstractCustomResourceWatcher.class.getName());
    ...
    // use via builder
    private AbstractCustomResourceWatcher(boolean isCrd, String namespace, String entityName, KubernetesClient client,
    ...
    public CompletableFuture<AbstractCustomResourceWatcher> build() {
        if (isCrd) {
            createCRDWatch();
        } else {
    ...
    private CompletableFuture<Watch> createCRDWatch() {
        CompletableFuture<Watch> cf = CompletableFuture.supplyAsync(() -> {
